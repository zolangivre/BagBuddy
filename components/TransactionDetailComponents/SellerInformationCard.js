...
import ListingConditionsCard from "./ListingConditions";
import { useQuery } from "@apollo/client/react";
import { AVERAGE_RATING } from "@/lib/graphql/reviews";
...
const SellerInformationCard = ({ item }) => {
  ...
  if (item) {
    ...
    sellerName =
      item?.listingInfo?.sellerUserInfo?.name ||
      item?.listingInfo?.userInfo?.name ||
      "???";
    buyerName =
      item?.buyerInfo?.name || item?.listingInfo?.userInfo?.name || "???";
  ...
  const revieweeId =
    role === "seller"
      ? item?.buyerId
      : item?.sellerId || item?.listingInfo?.userInfo?.sub;

  const { data: ratingData } = useQuery(AVERAGE_RATING, {
  ...
  return (
    <>
      ...
      >
        <View
          style={[styles.sellerCard, { backgroundColor: theme.flightCard }]}
        >
          <View style={styles.sellerAvatar}>
            <Text style={styles.sellerInitials}>
              {role === "seller" ? buyerName?.charAt(0) : sellerName?.charAt(0)}
            </Text>
          </View>
          <View style={styles.sellerInfo}>
            ...
            <Text style={theme.textStyles.bodyMedium}>
              ★ {averageRating ? averageRating.toFixed(1) : "N/A"} • {role === "seller" ? i18n.t("buyer") : i18n.t("seller")}
            </Text>
          </View>
          <ButtonIcon
            //   onPress={}
      ...
      <ListingConditionsCard transaction={item} />
    </>
  );
};
