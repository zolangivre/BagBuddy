...
const HomeCard = ({ item }) => {
  ...
  // userInfo vient du schéma GraphQL (UserInfoView), pas du /userinfo de
  // Keycloak : ses champs sont en camelCase.
  const initials = item.userInfo?.givenName
    ? item.userInfo.givenName.charAt(0).toUpperCase() +
      (item.userInfo.familyName?.charAt(0).toUpperCase() ?? "")
    : "NN";
