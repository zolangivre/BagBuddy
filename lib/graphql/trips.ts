...
/**
 * Nombre d'annonces pour un filtre, sans les annonces : c'est ce qu'affiche le
 * bouton de la feuille de filtres pendant qu'on les règle.
 */
export const SEARCH_TRIP_COUNT = gql`
  query SearchTripCount($filter: TripSearchInput) {
    searchTrips(filter: $filter, limit: 1) {
      totalCount
    }
  }
`;
